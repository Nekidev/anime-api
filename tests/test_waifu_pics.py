...
def test_get_random_image():
    ...
    """
    image = WaifuPicsAPI().get_random_image(ImageCategory.SFW.WAIFU)
...
def test_get_many_random_images():
    ...
    """
    images = WaifuPicsAPI().get_many_random_images(ImageCategory.SFW.WAIFU)
...
def test_get_many_random_images_with_exclude():
    """
    Test the get_many_random_images method with exclude filter
    """
    image_url = "https://i.waifu.pics/tLc5RfN.png" # Cute image btw
    images = WaifuPicsAPI().get_many_random_images(ImageCategory.SFW.WAIFU, exclude=[image_url])
    assert isinstance(images, list)
    assert len(images) == 30
    for image in images:
        assert isinstance(image, Image)
